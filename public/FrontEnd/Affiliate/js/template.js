...
// Change Navbar color based on the color behind it
...
function parseGradient(gradient) {
    const regex = /linear-gradient\(([\d.]+deg),\s*(#[0-9A-Fa-f]{6}|rgb\(\d{1,3}, \d{1,3}, \d{1,3}\))\s*([\d.]+%),\s*(#[0-9A-Fa-f]{6}|rgb\(\d{1,3}, \d{1,3}, \d{1,3}\))\s*([\d.]+%)\)/;
    const match = gradient.match(regex);

    if (match) {
        const color1 = match[2].startsWith("#") ? match[2] : rgbToHex(match[2]);
        const color2 = match[4].startsWith("#") ? match[4] : rgbToHex(match[4]);
...
function gradientsAreEqual(gradient1, gradient2) {
    if (!gradient1 || !gradient2) {
        return false;
    }

    if (gradient1.angle !== gradient2.angle || gradient1.colorStops.length !== gradient2.colorStops.length) {
        return false;
    }

    for (let i = 0; i < gradient1.colorStops.length; i++) {
        if (gradient1.colorStops[i].color.toLowerCase() !== gradient2.colorStops[i].color.toLowerCase() ||
            gradient1.colorStops[i].position !== gradient2.colorStops[i].position) {
            return false;
        }
    }
...
document.addEventListener("DOMContentLoaded", function () {
    const element = document.getElementById("changeNavbarColor");
    const container = document.querySelector(".content-bg");

    if(container){
        const containerColor = window.getComputedStyle(container).getPropertyValue("background-image");
        const referenceGradient = parseGradient("linear-gradient(135.04deg, #083D3D 0.52%, #041C1C 64.67%)");
        const containerGradient = parseGradient(containerColor);

        if (gradientsAreEqual(containerGradient, referenceGradient)) {
            element.style.backgroundColor = 'transparent';
        } else {
            element.style.backgroundColor = "#0E5959";
        }

        // console.log(containerColor + ' -o- ' + JSON.stringify(containerGradient) + ' -o- ' + JSON.stringify(referenceGradient));
    }else{
        element.style.backgroundColor = "#0E5959";
    }
...
function updateNavbarColor() {
    const element = document.getElementById("changeNavbarColor");
    const container = document.querySelector(".content-bg");
    const scrollThreshold = 50; // Change this value to the desired scroll position

    if (window.scrollY > scrollThreshold) {
        element.style.backgroundColor = "#752092"; // Change to the desired color when scrolling
    } else if (container) {
        const containerColor = window.getComputedStyle(container).getPropertyValue("background-image");
        const referenceGradient = parseGradient("linear-gradient(135.04deg, #083D3D 0.52%, #041C1C 64.67%)");
        const containerGradient = parseGradient(containerColor);
        ...
        if (gradientsAreEqual(containerGradient, referenceGradient)) {
            ...
            element.style.backgroundColor = "#752092";
        }
    } else {
        element.style.backgroundColor = "#752092";
    }
